...
def get_mentors(request, year):
    ...
    if len(mentors)>0:
        res_data    = MentorSerializer(mentors, many=True,context={'request':request}).data
...
def generate_spreadsheet(request):
    ...
    writer.writerow(['Name','Contact','Email','Details','Description','Profile_Pic',
                            'Flag','Year'])

    mentors = Mentor.objects.all().values_list('name','contact','email','details','description','profile_pic',
                            'flag','year')
