...
class team extends Component {
    ...
    members_html = (title, members, type) => {
        ...
        members = members.map(member => {
            let social_links =
                <Fragment>
                    {member.linkedin===null ? null : (<a className="profile_links" style={{margin:"0 15px"}} target="_blank" rel="noopener noreferrer" href={member.linkedin}><i className="fab fa-linkedin-in"></i></a>)}
                    
                    {member.facebook===null ? null: (<a className="profile_links" style={{margin:"0 15px"}} target="_blank" rel="noopener noreferrer" href={member.facebook}><i className="fab fa-facebook-f"></i></a>)}
                </Fragment>

            if (['FCT', 'HCD', 'DIR'].indexOf(member.member_type) > -1) {
                social_links = member.profile_url===null ? null : (<a className="profile_links" target="_blank" rel="noopener noreferrer" style={{color:"white"}} href={member.profile_url}><i className="fa fa-link"></i></a>)
            }
